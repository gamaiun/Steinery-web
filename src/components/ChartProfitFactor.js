...
const Chart = ({ points, trades }) => {
  ...
  useEffect(() => {
    ...
    const windowColors = {
      ...
      velvet: {
        gradient:
          "linear-gradient(135deg, #8BC6EC 0%, #9599E2 98%, #ffffff 99%)",
      },
    }
        ...
        datasets: [
          {
            ...
            borderColor: gradient, // Use the gradient
            data: drawdownData,
          },
        ],
      },
...
const ChartProfitFactor = ({ profitFactor, trades }) => {
  return (
    <AppContainer>
      <Chart points={profitFactor} trades={trades} />
    </AppContainer>
  )
}

export default ChartProfitFactor
